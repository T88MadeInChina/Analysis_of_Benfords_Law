...
REFERENZINTERVALL_FILE = 'referenzintervall.csv'
...
def leading_digit_analysis(data):
    ...
    # data = data[data.str.replace(',', '').str.isdigit()]
    # leading_digits = data.str[0].str[0].astype(int)


    # leading_digits = data.str.split('.').str[0].str[0].astype(int)
    data = data[data.str.replace(',', '').str.isdigit()]
    leading_digits = data.str.split(',').str[0].str[0].astype(int)
    leading_digits = leading_digits[leading_digits != 0]  # Filter out cases where the first digit is 0
...
def index():
    ...
    # Read the referenzintervall.csv file
...
def analyze():
    try:
        ...
        # Clean up non-numeric values and convert to floating point numbers
    ...
    try:
        ...
        if 'chi_square_test' in analysis_methods:
            ...
            if chi2_stat > 16.9:
                results['chi_square_test'][
                    'result'] = "With 8 degrees of freedom, the Chi-Square critical value at a 5% significance level is 15.51. Chi-Square value is greater than 15.51. The data does not conform to Benford's Law."
            else:
                results['chi_square_test'][
                    'result'] = "With 8 degrees of freedom, the Chi-Square critical value at a 5% significance level is 15.51. Chi-Square value is less than or equal to 15.51. The data conforms to Benford's Law."
        ...
        if 'z_test' in analysis_methods:
            ...
            if significant_digits:
                results['z_test']['result'] = f"With 8 degrees of freedom, the z_score critical value at a 5% significance level is 1.96.Digits {significant_digits} do not conform to Benford's Law."
            else:
                results['z_test']['result'] = "With 8 degrees of freedom, the z_score critical value at a 5% significance level is 1.96.All digits conform to Benford's Law."
        ...
        if 'regression_measure' in analysis_methods:
            ...
            observed_proportions = digit_freq
            fig = go.Figure()


            actual_data_color = 'blue'
            regression_line_color = 'blue'

            # Add Actual Data and Regression Line
            fig.add_trace(go.Scatter(x=benford_proportions, y=observed_proportions, mode='markers', name="Actual Data",
                                     marker=dict(color=actual_data_color)))
            fig.add_trace(go.Scatter(x=benford_proportions, y=slope * benford_proportions + intercept, mode='lines',
                                     name='Regression Line', line=dict(color=regression_line_color)))

            # Add ideal line segments
            fig.add_trace(go.Scatter(x=benford_proportions, y=benford_proportions, mode='lines', name='Ideal Line',
                                     line=dict(color='red')))

            fig.update_layout(
                title='Saville Regression Measure',
                xaxis_title='Theoretical Benford',
                yaxis_title='Actually obseved',
...
def interval_analyze():
    try:
        ...
        # Find the corresponding interval based on the age range entered by the user
        ...
        # Analyze all data with an age in the interval range
